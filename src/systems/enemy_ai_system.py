...
def update_enemy_state(enemy, player, level):
    '''Переключает FSM врага между атакой, погоней, поиском и ожиданием'''
...
def get_target_cell(enemy, player, level):
    '''Возвращает целевую клетку для текущего состояния врага'''
...
def get_random_idle_cell(enemy, level):
    '''Выбирает случайную соседнюю клетку для блуждания врага'''
...
def open_next_door(enemy, level):
    '''Открывает дверь, если она является следующей клеткой пути'''
    if not enemy.path:
        return False
